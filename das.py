...
def das(iqraw, tA, tB, fs, fd, A=None, B=None, apoA=1, apoB=1, interp="cubic"):
    ...
    size, as long as the innermost dimension matches na or nb, respectively. Another
    ...
    def bbint(iq, t):
        ...
        return iqfoc * jnp.exp(2j * jnp.pi * fd * t) # 对插值后的 IQ 数据进行基带调制
    ...
    return jnp.tensordot(A, jnp.stack([das_b(x) for x in zip(iqraw, tA)]) * apoA, (-1, 0))
